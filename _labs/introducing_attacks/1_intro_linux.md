...
## Optional: Setting Up a Home Lab {#optional-setting-up-a-home-lab}

While you can access Hacktivity remotely, setting up your own local lab environment with VMs can provide valuable additional practice. This is entirely optional—all exercises can be completed within the Hacktivity platform.

> Warning: **Ensure you are not acting illegally**. Many tasks in these labs could be considered illegal if targeted at systems you don't own or have explicit permission to test. Any personal lab environment should be completely isolated from external networks. When in doubt, work exclusively within Hacktivity's secure environment.

If you choose to set up a local environment, consider using VMware Player or VirtualBox. Note that personal setups are at your own risk, and support is limited to the Hacktivity platform.
