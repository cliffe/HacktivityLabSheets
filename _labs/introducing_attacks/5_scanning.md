...
\==VM: Interact with the Kali VM==. (Click the launch icon after the VMs have started): ![VM Launch Icon][vm_launch_icon] ==action: Login with username "kali", password "kali".==
...
The responses to the commands above also include DNS name resolution against each IP address. This by itself can be very informative. ==action: To only list the hosts along with their hostnames run a "List scan"[1](#user-content-fn-1):==
...
![][tcp_handshake] *TCP three-way handshake*
...
![][zenmap] [*Zenmap: a GUI for Nmap*](http://nmap.org/zenmap/)

> Note: Kali Linux recently removed Zenmap (and all python 2 programs), however, the screenshot above illustrates that these GUI programs provide an alternative interface. If you wish to learn more about it, you can find information at https://nmap.org/zenmap/. You could also spend a few minutes looking into alternative GUIs.
...
> Flag Tip: As you scan, pay close attention to the services and version banners you turn up. Recognising a known-vulnerable service from scanning (such as distcc) can be a route to a flag.
...
[vm_launch_icon]: {{ site.baseurl }}/assets/images/common/vm_launch_icon.png [tcp_handshake]: {{ site.baseurl }}/assets/images/introducing_attacks/5_scanning/tcp_handshake.png [zenmap]: {{ site.baseurl }}/assets/images/introducing_attacks/5_scanning/zenmap.png

## Footnotes

1. Note that the list scan below may take a while to run (around 5 ½ minutes). Feel free to leave it running and carry on working on another tab if you prefer. [↩](#user-content-fnref-1)
2. Or have the appropriate Linux capability, although understanding this is outside of the scope of this lab. [↩](#user-content-fnref-2)
3. For a technical description of how Nmap’s OS detection works: http://nmap.org/book/osdetect-methods.html [↩](#user-content-fnref-3)
