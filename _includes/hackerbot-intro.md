{%- comment -%} Shared "Meet Hackerbot!" section for Hackerbot labs. Single point of maintenance: edit this file rather than the copy in each lab sheet.

Parameters: role - the clause after "a chatbot who will". Default: "attack your system". e.g. role="task you to monitor the network and will attack your systems" chat - "full" : list of opening messages to try (first Hackerbot lab in a series) "hello" : just send "hello" omitted : no chat step (later labs, where students already know) {%- endcomment -%}
...
- Send "help"
- Send "list"
- Send "hello" {% elsif include.chat == "hello" %} ==VM: On the desktop VM==, ==action: open Firefox and go to `hackerbot:8080`, then send "hello" to Hackerbot:== {% endif %}
